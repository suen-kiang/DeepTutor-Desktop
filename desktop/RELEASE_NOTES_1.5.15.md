# DeepTutor Desktop 1.5.15

基于官方 DeepTutor **v1.5.15** 的 macOS 桌面封装（Electron 瘦包，官方源码零改动）。

## 本版变化

- 同步官方上游 **v1.5.14 → v1.5.15**（新增 question-bank 题库、whisper 语音等能力；补装新增默认依赖 `pageindex`）。
- 桌面层版本号体系调整：**从 `desktop-0.2.0` 改为 `desktop-v1.5.15`**（后缀跟随官方版本号）。
